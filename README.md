# MyCourse

## Description
A basic mobile-friendly application that allows you to create a school schedule for yourself, including terms, classes, and information about said classes.

## Installation
Clone the repository and open the solution in Visual Studio. Upon building, you may be asked to install certain NuGet packages. This is expected. Use Visual Studio's built in building for the solution. Set up an android emulator to work directly with the solution. For testing, use the testing tab built into Visual Studio.

## Contributing
If downloading this directly to your machine, be sure to keep in mind the 260 character limit in file paths, and maybe put this at a higher level (such as `C:\Users\username\Dev\<this-repo>`).

## Authors and acknowledgment
Jtnoble

## License
N/A

## Project status
WIP
